...
def build_normal_matrix(m,n):
    ...
    #A = np.random.uniform(0,1000,(m,n))

    A_norm = np.linalg.norm(A, axis=1, ord=2)#A每个行向量求向量的2范数即aj
...
# The first iteration
    ...
    for k in range(iter_max-1):
        ...
        np.delete(A_norm1, r)
        del rr[r]
        A_csum1 = np.cumsum(A_norm1)
        rand_num = np.random.uniform(0, A_csum1[-1])
        r = np.min(np.where(A_csum1 > rand_num))
        r = rr[r-1]
        a = A[r].reshape((1, n))
        v = b[r] - np.dot(a, x)
        v_norm = np.linalg.norm(a, 2) ** 2
